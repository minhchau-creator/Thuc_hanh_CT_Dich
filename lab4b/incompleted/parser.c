...
void compileIfSt(void) {
  ...
  if (lookAhead->tokenType == KW_ELSE) {
    // Nhảy qua phần else khi đã thực hiện then
    jInstruction = genJ(DC_VALUE);

    // Cập nhật FJ để nhảy tới phần else
    updateFJ(fjInstruction, getCurrentCodeAddress());

    eat(KW_ELSE);
    compileStatement();

    // Cập nhật J để nhảy ra khỏi if-else
