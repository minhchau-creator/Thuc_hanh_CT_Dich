...
int sizeOfType(Type *type) {
  // TODO
  // kiểu integer/char: 4 bytes = 1 word
  if (type->typeClass == TP_INT || type->typeClass == TP_CHAR)
    return 1;
  // kiểu mảng: số phần tử * kích thước kiểu phần tử
  if (type->typeClass == TP_ARRAY)
...
void declareObject(Object *obj) {
  if (symtab->currentScope == NULL) {
    ...
    switch (obj->kind) {
    case OBJ_VARIABLE:
      // Thiết lập scope và localOffset cho biến, cập nhật frameSize, thêm vào objList của currentScope
      obj->varAttrs->scope = symtab->currentScope; // Thiết lập scope
      obj->varAttrs->localOffset = symtab->currentScope->frameSize; // Thiết lập localOffset
      symtab->currentScope->frameSize += sizeOfType(obj->varAttrs->type);  // Cập nhật frameSize
      addObject(&(symtab->currentScope->objList), obj); // add vào currentScope của objList
      break;
    ...
    case OBJ_PARAMETER:
      ...
      if (symtab->currentScope->owner->kind == OBJ_FUNCTION) {  // Thêm vào paramList
    ...
    case OBJ_FUNCTION:
      obj->funcAttrs->scope->outer = symtab->currentScope;  // Liên kết scope outer
      addObject(&(symtab->currentScope->objList), obj);
      break;
